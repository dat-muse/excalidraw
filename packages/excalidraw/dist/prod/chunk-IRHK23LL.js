import { a as J } from "./chunk-A66AFZZU.js";
import { d as _ } from "./chunk-SRAX5OIU.js";
import j6 from "png-chunk-text";
import ko from "png-chunks-encode";
import z6 from "png-chunks-extract";
import { EXPORT_DATA_TYPES as jo, MIME_TYPES as be } from "@excalidraw/common";
import { nanoid as Bo } from "nanoid";
import {
  IMAGE_MIME_TYPES as vo,
  MIME_TYPES as O,
  bytesToHexString as Zo,
  isPromiseLike as So,
} from "@excalidraw/common";
import { clearElementsForExport as Wo } from "@excalidraw/element";
import {
  COLOR_PALETTE as U5,
  ARROW_TYPE as e5,
  DEFAULT_ELEMENT_PROPS as Z,
  DEFAULT_FONT_FAMILY as r5,
  DEFAULT_FONT_SIZE as a5,
  DEFAULT_TEXT_ALIGN as d5,
  DEFAULT_GRID_SIZE as c5,
  EXPORT_SCALES as f5,
  STATS_PANELS as ne,
  THEME as t5,
  DEFAULT_GRID_STEP as o5,
  isTestEnv as i5,
} from "@excalidraw/common";
var s5 = f5.includes(devicePixelRatio) ? devicePixelRatio : 1,
  oU = () => ({
    showWelcomeScreen: !1,
    theme: t5.LIGHT,
    collaborators: new Map(),
    currentChartType: "bar",
    currentItemBackgroundColor: Z.backgroundColor,
    currentItemEndArrowhead: "arrow",
    currentItemFillStyle: Z.fillStyle,
    currentItemFontFamily: r5,
    currentItemFontSize: a5,
    currentItemOpacity: Z.opacity,
    currentItemRoughness: Z.roughness,
    currentItemStartArrowhead: null,
    currentItemStrokeColor: Z.strokeColor,
    currentItemRoundness: i5() ? "sharp" : "round",
    currentItemArrowType: e5.round,
    currentItemStrokeStyle: Z.strokeStyle,
    currentItemStrokeWidth: Z.strokeWidth,
    currentItemTextAlign: d5,
    ...
    stats: { open: !1, panels: ne.generalStats | ne.elementProperties },
  ...
  b5 = ((U) => U)({
    ...
    lockedMultiSelections: { browser: !0, export: !0, server: !0 },
    activeLockedId: { browser: !1, export: !1, server: !1 },
  }),
  le = (U, e) => {
    let r = {};
...
var gU = (U) => le(U, "export"),
  Ee = (U) => le(U, "server"),
  ei = ({ activeTool: U }) => U.type === "eraser",
  ri = ({ activeTool: U }) => U.type === "hand";
var RU = class extends Error {
    constructor(e = "Couldn't export canvas.", r = "CANVAS_ERROR") {
      super(), (this.name = r), (this.message = e);
    }
  },
  ...
  H = class extends Error {
    constructor(r = "Image Scene Data Error", a = "IMAGE_SCENE_DATA_ERROR") {
      super(r);
      _(this, "code");
      (this.name = "EncodingError"), (this.code = a);
    }
  },
  rU = class extends Error {
    constructor(r = "Worker URL is not defined!", a = "WORKER_URL_NOT_DEFINED") {
      super(r);
      _(this, "code");
      (this.name = "WorkerUrlNotDefinedError"), (this.code = a);
    }
  },
  aU = class extends Error {
    constructor(r = "Worker has to be in a separate chunk!", a = "WORKER_IN_THE_MAIN_CHUNK") {
      super(r);
      _(this, "code");
      (this.name = "WorkerInTheMainChunkError"), (this.code = a);
    }
  },
...
import {
  isSomeElementSelected as xi,
  getElementsWithinSelection as Ii,
  getSelectedElements as mi,
  getTargetElements as Ji,
} from "@excalidraw/element";
import { getVisibleElements as n5 } from "@excalidraw/element";
import {
  sceneCoordsToViewportCoords as ye,
  viewportCoordsToSceneCoords as l5,
} from "@excalidraw/common";
import { getClosestElementBounds as E5 } from "@excalidraw/element";
import { getCommonBounds as p5 } from "@excalidraw/element";
var y5 = (U, e) => {
    let [r, a, c, t] = e,
      { x: d, y: f } = ye({ sceneX: r, sceneY: a }, U),
      { x: i, y: s } = ye({ sceneX: c, sceneY: t }, U);
    return i - d > U.width || s - f > U.height;
  },
  g5 = ({ scenePoint: U, viewportDimensions: e, zoom: r, offsets: a }) => {
    let c = (e.width - (a?.right ?? 0)) / 2 / r.value - U.x;
    c += (a?.left ?? 0) / 2 / r.value;
    let t = (e.height - (a?.bottom ?? 0)) / 2 / r.value - U.y;
    return (t += (a?.top ?? 0) / 2 / r.value), { scrollX: c, scrollY: t };
  },
  MU = (U, e) => {
    if (((U = n5(U)), !U.length)) return { scrollX: 0, scrollY: 0 };
    let [r, a, c, t] = p5(U);
    y5(e, [r, a, c, t]) &&
      ([r, a, c, t] = E5(U, l5({ clientX: e.scrollX, clientY: e.scrollY }, e)));
    let d = (r + c) / 2,
      f = (a + t) / 2;
    return g5({
      scenePoint: { x: d, y: f },
      viewportDimensions: { width: e.width, height: e.height },
      zoom: e.zoom,
    });
  };
import {
  hasBackground as Ni,
  hasStrokeWidth as Di,
  hasStrokeStyle as qi,
  canHaveArrowheads as Fi,
  canChangeRoundness as Yi,
} from "@excalidraw/element";
import { MAX_ZOOM as R5, MIN_ZOOM as $5 } from "@excalidraw/common";
import { clamp as BU, round as u5 } from "@excalidraw/math";
var vU = (U) => BU(u5(U, 6), $5, R5),
  ZU = (U) => BU(Math.round(U), 1, 100),
  SU = (U) => BU(Math.round(U), 1, 100);
import Y6 from "roughjs/bin/rough";
import {
  DEFAULT_EXPORT_PADDING as _6,
  FRAME_STYLE as j,
  FONT_FAMILY as Z7,
  SVG_NS as P,
  THEME as q6,
  THEME_FILTER as S7,
  MIME_TYPES as L6,
  EXPORT_DATA_TYPES as W7,
  arrayToMap as VU,
  distance as F6,
  getFontString as Q7,
  toBrandedType as fe,
} from "@excalidraw/common";
import { getCommonBounds as A7, getElementAbsoluteCoords as G7 } from "@excalidraw/element";
import { getInitializedImageElements as C7, updateImageCache as P7 } from "@excalidraw/element";
import { newElementWith as k7 } from "@excalidraw/element";
import { isFrameLikeElement as V6 } from "@excalidraw/element";
import {
  getElementsOverlappingFrame as j7,
  getFrameLikeElements as z7,
  getFrameLikeTitle as Uo,
  getRootElements as w6,
} from "@excalidraw/element";
import { syncInvalidIndices as eo } from "@excalidraw/element";
import { newTextElement as ro } from "@excalidraw/element";
import { deflate as x5, inflate as I5 } from "pako";
import { ENCRYPTION_KEY_BITS as Vi } from "@excalidraw/common";
var iU = (U) => {
    let e =
        typeof U == "string"
          ...
          : new Uint8Array(U),
      r = "";
    for (let a of e) r += String.fromCharCode(a);
    return r;
  },
  ge = (U) => {
    let e = new ArrayBuffer(U.length),
      r = new Uint8Array(e);
    for (let a = 0, c = U.length; a < c; a++) r[a] = U.charCodeAt(a);
    return e;
  },
  Re = (U) => new TextDecoder("utf-8").decode(ge(U)),
  uU = (U, e = !1) => (e ? window.btoa(U) : window.btoa(iU(U))),
  xU = (U, e = !1) => (e ? window.atob(U) : Re(window.atob(U)));
var IU = ({ text: U, compress: e }) => {
    let r;
    if (e !== !1)
      try {
        r = iU(x5(U));
    ...
    return { version: "1", encoding: "bstring", compressed: !!r, encoded: r || iU(U) };
  },
  mU = (U) => {
    let e;
    switch (U.encoding) {
      case "bstring":
        e = U.compressed ? U.encoded : Re(U.encoded);
        break;
      default:
        throw new Error(`decode: unknown encoding "${U.encoding}"`);
    }
    return U.compressed ? I5(new Uint8Array(ge(e)), { to: "string" }) : e;
  };
import {
  DEFAULT_FILENAME as N5,
  EXPORT_DATA_TYPES as JU,
  getExportSource as Ie,
  MIME_TYPES as me,
  VERSIONS as Je,
} from "@excalidraw/common";
import { clearElementsForDatabase as D5, clearElementsForExport as q5 } from "@excalidraw/element";
import { fileOpen as m5, fileSave as J5, supported as ue } from "browser-fs-access";
import { EVENT as dU, MIME_TYPES as K5, debounce as T5 } from "@excalidraw/common";
var $e = 500,
  xe = (U) => {
    let e = U.extensions?.reduce((a, c) => (a.push(K5[c]), a), []),
      r = U.extensions?.reduce(
        (a, c) => (c === "jpg" ? a.concat(".jpg", ".jpeg") : a.concat(`.${c}`)),
        []
      );
    return m5({
      ...
      extensions: r,
      mimeTypes: e,
      multiple: U.multiple ?? !1,
      legacySetup: (a, c, t) => {
        let d = T5(c, $e),
          f = () => {
            i(),
              document.addEventListener(dU.KEYUP, d),
              document.addEventListener(dU.POINTER_UP, d),
              d();
          },
          i = () => {
            if (t.files?.length) {
              let b = U.multiple ? [...t.files] : t.files[0];
              a(b);
            }
          };
        requestAnimationFrame(() => {
          window.addEventListener(dU.FOCUS, f);
        });
        ...
        return (b) => {
          clearInterval(s),
            d.cancel(),
            window.removeEventListener(dU.FOCUS, f),
            document.removeEventListener(dU.KEYUP, d),
            document.removeEventListener(dU.POINTER_UP, d),
            b && (console.warn("Opening the file was canceled (legacy-fs)."), b(new $U()));
        };
      },
    });
  },
...
var F5 = (U, e) => {
    let r = {};
    for (let a of U)
      !a.isDeleted && "fileId" in a && a.fileId && e[a.fileId] && (r[a.fileId] = e[a.fileId]);
    return r;
  },
  QU = (U, e, r, a) => {
    let c = {
      type: JU.excalidraw,
      version: Je.excalidraw,
      source: Ie(),
      elements: a === "local" ? q5(U) : D5(U),
      appState: a === "local" ? gU(e) : Ee(e),
      files: a === "local" ? F5(U, r) : void 0,
    };
    return JSON.stringify(c, null, 2);
  },
  U2 = async (U, e, r, a = e.name || N5) => {
    let c = QU(U, e, r, "local"),
      t = new Blob([c], { type: me.excalidraw });
    return {
      fileHandle: await WU(t, {
        name: a,
        extension: "excalidraw",
        description: "Excalidraw file",
        fileHandle: Te(e.fileHandle) ? null : e.fileHandle,
      }),
    };
  },
  e2 = async (U, e) => {
    let r = await xe({ description: "Excalidraw files" });
    return Ne(await GU(r), U, e, r.handle);
  },
  Ke = (U) =>
    U?.type === JU.excalidraw &&
    (!U.elements || (Array.isArray(U.elements) && (!U.appState || typeof U.appState == "object"))),
  AU = (U) =>
    typeof U == "object" &&
    U &&
    U.type === JU.excalidrawLibrary &&
    (U.version === 1 || U.version === 2),
  Y5 = (U) => {
    let e = {
      type: JU.excalidrawLibrary,
      version: Je.excalidrawLibrary,
      source: Ie(),
      libraryItems: U,
    };
  ...
  r2 = async (U) => {
    let e = Y5(U);
    await WU(new Blob([e], { type: me.excalidrawlib }), {
      name: "library",
...
import {
  FONT_FAMILY as zU,
  FONT_FAMILY_FALLBACKS as NU,
  CJK_HAND_DRAWN_FALLBACK_FONT as DU,
  WINDOWS_EMOJI_FALLBACK_FONT as r7,
  getFontFamilyFallbacks as a7,
} from "@excalidraw/common";
import { getContainerElement as d7 } from "@excalidraw/element";
import { charWidth as c7 } from "@excalidraw/element";
import { containsCJK as f7 } from "@excalidraw/element";
import {
  FONT_METADATA as r6,
  getFontString as a6,
  PromisePool as d6,
...
import { ShapeCache as f6 } from "@excalidraw/element";
import { isTextElement as Ue } from "@excalidraw/element";
var De = "./fonts/Cascadia/CascadiaCode-Regular.woff2";
var qe = [{ uri: De }];
var Fe = "./fonts/ComicShanns/ComicShanns-Regular-279a7b317d12eb88de06167bd672b4b4.woff2";
var Ye = "./fonts/ComicShanns/ComicShanns-Regular-6e066e8de2ac57ea9283adb9c24d7f0c.woff2";
var _e = "./fonts/ComicShanns/ComicShanns-Regular-dc6a8806fa96795d7b3be5026f989a17.woff2";
var Le = "./fonts/ComicShanns/ComicShanns-Regular-fcb0fc02dcbee4c9846b3e2508668039.woff2";
var Ve = [
  {
    uri: Fe,
    descriptors: {
  ...
  { uri: Ye, descriptors: { unicodeRange: "U+3bb" } },
];
import { LOCAL_FONT_PROTOCOL as X5 } from "@excalidraw/common";
var we = [{ uri: X5 }];
import { promiseTry as Z5, LOCAL_FONT_PROTOCOL as S5 } from "@excalidraw/common";
import { isServerEnv as H5, promiseTry as Xe } from "@excalidraw/common";
import { debounce as h5 } from "@excalidraw/common";
var CU = class {
    constructor(e) {
      _(this, "instance");
      _(this, "debounceTerminate");
      this.instance = new Worker(e, { type: "module" });
    }
  },
  KU = class U {
    constructor(e, r) {
      _(this, "idleWorkers", new Set());
      _(this, "workerUrl");
      _(this, "workerTTL");
      (this.workerUrl = e), (this.workerTTL = r.ttl || 1e3);
    }
    static create(e, r = {}) {
      if (!e) throw new rU();
      if (!import.meta.url || e.toString() === import.meta.url) throw new aU();
      return new U(e, r);
    }
    async postMessage(e, r) {
      let a,
        c = Array.from(this.idleWorkers).shift();
      return (
        c ? (this.idleWorkers.delete(c), (a = c)) : (a = await this.createWorker()),
        new Promise((t, d) => {
          (a.instance.onmessage = this.onMessageHandler(a, t)),
            (a.instance.onerror = this.onErrorHandler(a, d)),
            a.instance.postMessage(e, r),
            a.debounceTerminate(() =>
              d(new Error(`Active worker did not respond for ${this.workerTTL}ms!`))
            );
        })
      );
    }
    async clear() {
      for (let e of this.idleWorkers) e.debounceTerminate.cancel(), e.instance.terminate();
      this.idleWorkers.clear();
    }
    async createWorker() {
      let e = new CU(this.workerUrl);
      return (
        (e.debounceTerminate = h5((r) => {
          e.instance.terminate(),
            this.idleWorkers.has(e)
              ? (this.idleWorkers.delete(e),
    ...
    onMessageHandler(e, r) {
      return (a) => {
        e.debounceTerminate(), this.idleWorkers.add(e), r(a.data);
      };
    }
    onErrorHandler(e, r) {
      return (a) => {
        e.debounceTerminate(() => r(a)), e.debounceTerminate.flush(), this.clear();
      };
    }
  };
var Oe = typeof Worker < "u",
  he = async (U, e) => {
    let { Commands: r, subsetToBase64: a, toBase64: c } = await B5();
    return Oe
      ? Xe(async () => {
          try {
            let t = await v5(),
              d = U.slice(0),
              f = await t.postMessage(
                { command: r.Subset, arrayBuffer: d, codePoints: e },
                { transfer: [d] }
              );
            return c(f);
          } catch (t) {
            return (
              (Oe = !1),
              (H5() && (t instanceof rU || t instanceof aU)) ||
                console.error(
  ...
  PU = null,
  kU = null,
  M5 = async () => (PU || (PU = import("./subset-worker.chunk.js")), PU),
  B5 = async () => (kU || (kU = import("./subset-shared.chunk.js")), kU),
  jU = null,
  v5 = () => (
    jU ||
      (jU = Xe(async () => {
        let { WorkerUrl: U } = await M5();
        return KU.create(U);
      })),
    jU
  );
var cU = class cU {
  constructor(e, r, a) {
    _(this, "urls");
    _(this, "fontFace");
    this.urls = cU.createUrls(r);
    let c = this.urls.map((t) => `url(${t}) ${cU.getFormat(t)}`).join(", ");
    this.fontFace = new FontFace(e, c, { display: "swap", style: "normal", weight: "400", ...a });
  }
  toCSS(e) {
    if (!this.getUnicodeRangeRegex().test(e)) return;
    let r = Array.from(e).map((a) => a.codePointAt(0));
    return this.getContent(r).then(
      (a) => `@font-face { font-family: ${this.fontFace.family}; src: url(${a}); }`
    );
  }
  async getContent(e) {
    let r = 0,
      a = [];
    for (; r < this.urls.length; ) {
      let c = this.urls[r];
      try {
        let t = await this.fetchFont(c);
        return await he(t, e);
      } catch (t) {
        a.push(`"${c.toString()}" returned error "${t}"`);
      }
      r++;
    }
    return (
  ...
  fetchFont(e) {
    return Z5(async () => {
      let r = await fetch(e, { cache: "force-cache", headers: { Accept: "font/woff2" } });
      if (!r.ok) {
        let c = e instanceof URL ? e.toString() : "dataurl";
        throw new Error(`Failed to fetch "${c}": ${r.statusText}`);
      }
      return await r.arrayBuffer();
    });
  }
  getUnicodeRangeRegex() {
    let e = this.fontFace.unicodeRange
      .split(/,\s*/)
      .map((r) => {
        let [a, c] = r.replace("U+", "").split("-");
        return c ? `\\u{${a}}-\\u{${c}}` : `\\u{${a}}`;
      })
      .join("");
  ...
  static createUrls(e) {
    ...
    if (e.startsWith(S5)) return [];
    if (e.startsWith("http")) return [new URL(e)];
    let r = e.replace(/^\/+/, ""),
      a = [];
    if (typeof window.EXCALIDRAW_ASSET_PATH == "string") {
      let c = this.normalizeBaseUrl(window.EXCALIDRAW_ASSET_PATH);
      a.push(new URL(r, c));
    } else
      Array.isArray(window.EXCALIDRAW_ASSET_PATH) &&
        window.EXCALIDRAW_ASSET_PATH.forEach((c) => {
          let t = this.normalizeBaseUrl(c);
          a.push(new URL(r, t));
        });
    return a.push(new URL(r, cU.ASSETS_FALLBACK_URL)), a;
  }
  static getFormat(e) {
  ...
  static normalizeBaseUrl(e) {
    let r = e;
    return (
      /^\.?\//.test(r) &&
        (r = new URL(r.replace(/^\.?\/+/, ""), window?.location?.origin).toString()),
      (r = `${r.replace(/\/+$/, "")}/`),
      r
    );
  }
...
_(
  cU,
  "ASSETS_FALLBACK_URL",
  `https://esm.sh/${J.PKG_NAME ? `${J.PKG_NAME}@${J.PKG_VERSION}` : "excalidraw-custom"}/dist/prod/`
);
var TU = cU;
var He = "./fonts/Excalifont/Excalifont-Regular-349fac6ca4700ffec595a7150a0d1e1d.woff2";
var Me = "./fonts/Excalifont/Excalifont-Regular-3f2c5db56cc93c5a6873b1361d730c16.woff2";
var Be = "./fonts/Excalifont/Excalifont-Regular-41b173a47b57366892116a575a43e2b6.woff2";
var ve = "./fonts/Excalifont/Excalifont-Regular-623ccf21b21ef6b3a0d87738f77eb071.woff2";
var Ze = "./fonts/Excalifont/Excalifont-Regular-a88b72a24fb54c9f94e3b5fdaa7481c9.woff2";
var Se = "./fonts/Excalifont/Excalifont-Regular-b9dcf9d2e50a1eaf42fc664b50a3fd0d.woff2";
var We = "./fonts/Excalifont/Excalifont-Regular-be310b9bcd4f1a43f571c46df7809174.woff2";
var Qe = [
  {
    uri: Ze,
    descriptors: {
  ...
  { uri: Se, descriptors: { unicodeRange: "U+400-45f,U+490-491,U+2116" } },
  {
    uri: Be,
    descriptors: {
  ...
  { uri: ve, descriptors: { unicodeRange: "U+300-301,U+303" } },
];
import { LOCAL_FONT_PROTOCOL as j5 } from "@excalidraw/common";
var Ae = [{ uri: j5 }];
var Ge = "./fonts/Liberation/LiberationSans-Regular.woff2";
var Ce = [{ uri: Ge }];
import { GOOGLE_FONTS_RANGES as je } from "@excalidraw/common";
var Pe = "./fonts/Lilita/Lilita-Regular-i7dPIFZ9Zz-WBtRtedDbYE98RXi4EwSsbg.woff2";
var ke = "./fonts/Lilita/Lilita-Regular-i7dPIFZ9Zz-WBtRtedDbYEF8RXi4EwQ.woff2";
var ze = [
  { uri: Pe, descriptors: { unicodeRange: je.LATIN_EXT } },
  { uri: ke, descriptors: { unicodeRange: je.LATIN } },
];
import { GOOGLE_FONTS_RANGES as sU } from "@excalidraw/common";
var Ur = "./fonts/Nunito/Nunito-Regular-XRXI3I6Li01BKofiOc5wtlZ2di8HDIkhdTA3j6zbXWjgevT5.woff2";
var er = "./fonts/Nunito/Nunito-Regular-XRXI3I6Li01BKofiOc5wtlZ2di8HDIkhdTQ3j6zbXWjgeg.woff2";
var rr = "./fonts/Nunito/Nunito-Regular-XRXI3I6Li01BKofiOc5wtlZ2di8HDIkhdTk3j6zbXWjgevT5.woff2";
var ar = "./fonts/Nunito/Nunito-Regular-XRXI3I6Li01BKofiOc5wtlZ2di8HDIkhdTo3j6zbXWjgevT5.woff2";
var Vd = "./fonts/Xiaolai/Xiaolai-Regular-b7d203b051eff504ff59ddca7576b6a9.woff2";
var wd = "./fonts/Xiaolai/Xiaolai-Regular-4a38cc3e9cf104e69ba246d37f8cf135.woff2";
var Od = "./fonts/Xiaolai/Xiaolai-Regular-982b630266d87db93d2539affb1275c6.woff2";
var Xd = "./fonts/Xiaolai/Xiaolai-Regular-9592bfc861f07bcb8d75c196b370e548.woff2";
var hd = "./fonts/Xiaolai/Xiaolai-Regular-a7accba310e821da5505f71c03b76bdb.woff2";
var Hd = "./fonts/Xiaolai/Xiaolai-Regular-dac48066b5883d8b4551fc584f0c2a3e.woff2";
var Md = "./fonts/Xiaolai/Xiaolai-Regular-a1f916d6039285c4ffb900cd654e418f.woff2";
var Bd = "./fonts/Xiaolai/Xiaolai-Regular-95bfd249da4902577b4b7d76ebdd0b44.woff2";
var vd = "./fonts/Xiaolai/Xiaolai-Regular-93fc8f28a33234bcadf1527cafabd502.woff2";
var Zd = "./fonts/Xiaolai/Xiaolai-Regular-903bb6865f3452e2fda42e3a25547bc5.woff2";
var Sd = "./fonts/Xiaolai/Xiaolai-Regular-4aca6a43e59aceee2166b0c7e4e85ef1.woff2";
var Wd = "./fonts/Xiaolai/Xiaolai-Regular-24476a126f129212beb33f66853ea151.woff2";
var Qd = "./fonts/Xiaolai/Xiaolai-Regular-1b611157cd46bb184d4fa4dae2d6a2b8.woff2";
var Ad = "./fonts/Xiaolai/Xiaolai-Regular-56a32a7689abd0326e57c10c6c069bb4.woff2";
var Gd = "./fonts/Xiaolai/Xiaolai-Regular-3cc70dbb64df5b21f1326cc24dee2195.woff2";
var Cd = "./fonts/Xiaolai/Xiaolai-Regular-e2ead7ea7da0437f085f42ffc05f8d13.woff2";
var Pd = "./fonts/Xiaolai/Xiaolai-Regular-97f7f48ce90c9429bf32ae51469db74d.woff2";
var kd = "./fonts/Xiaolai/Xiaolai-Regular-24a21c1e4449222e8d1898d69ff3a404.woff2";
var jd = "./fonts/Xiaolai/Xiaolai-Regular-726303e0774b4e678bff8c2deb6ca603.woff2";
var zd = "./fonts/Xiaolai/Xiaolai-Regular-5a7fac4b8b23a6e4e5ba0c9bf1756c91.woff2";
var U6 = "./fonts/Xiaolai/Xiaolai-Regular-2b7441d46298788ac94e610ffcc709b6.woff2";
var e6 = [
  {
    uri: pr,
    descriptors: {
  ...
  { uri: Rr, descriptors: { unicodeRange: "U+d7eb-d7fb,U+f900-f9b7" } },
  { uri: ur, descriptors: { unicodeRange: "U+d6f2-d7a3,U+d7b0-d7c6,U+d7cb-d7ea" } },
  { uri: Ir, descriptors: { unicodeRange: "U+d609-d6f1" } },
  { uri: mr, descriptors: { unicodeRange: "U+d520-d608" } },
  { uri: xr, descriptors: { unicodeRange: "U+d437-d51f" } },
  { uri: Jr, descriptors: { unicodeRange: "U+d34e-d436" } },
  { uri: Tr, descriptors: { unicodeRange: "U+d265-d34d" } },
  { uri: Nr, descriptors: { unicodeRange: "U+d17c-d264" } },
  { uri: gr, descriptors: { unicodeRange: "U+d093-d17b" } },
  { uri: Dr, descriptors: { unicodeRange: "U+cfaa-d092" } },
  { uri: qr, descriptors: { unicodeRange: "U+cec1-cfa9" } },
  { uri: sr, descriptors: { unicodeRange: "U+cdd8-cec0" } },
  { uri: yr, descriptors: { unicodeRange: "U+ccf1-cdd7" } },
  { uri: Fr, descriptors: { unicodeRange: "U+cc08-ccf0" } },
  { uri: ir, descriptors: { unicodeRange: "U+cb43-cc07" } },
  { uri: Yr, descriptors: { unicodeRange: "U+ca83-cb42" } },
  { uri: _r, descriptors: { unicodeRange: "U+c9a1-ca82" } },
  { uri: Lr, descriptors: { unicodeRange: "U+c8b8-c9a0" } },
  { uri: Vr, descriptors: { unicodeRange: "U+c7cf-c8b7" } },
  { uri: wr, descriptors: { unicodeRange: "U+c6e6-c7ce" } },
  { uri: Or, descriptors: { unicodeRange: "U+c5fd-c6e5" } },
  { uri: Xr, descriptors: { unicodeRange: "U+c514-c5fc" } },
  { uri: hr, descriptors: { unicodeRange: "U+c42b-c513" } },
  { uri: Hr, descriptors: { unicodeRange: "U+c341-c34e,U+c350-c42a" } },
  { uri: Mr, descriptors: { unicodeRange: "U+c258-c340" } },
  { uri: Br, descriptors: { unicodeRange: "U+c16f-c257" } },
  { uri: vr, descriptors: { unicodeRange: "U+c086-c16e" } },
  { uri: Zr, descriptors: { unicodeRange: "U+bf9d-c085" } },
  { uri: Kr, descriptors: { unicodeRange: "U+beb4-bf9c" } },
  { uri: Sr, descriptors: { unicodeRange: "U+bdcb-beb3" } },
  { uri: Wr, descriptors: { unicodeRange: "U+bce2-bdca" } },
  { uri: Qr, descriptors: { unicodeRange: "U+bbf9-bce1" } },
  { uri: Ar, descriptors: { unicodeRange: "U+bb10-bbf8" } },
  { uri: Gr, descriptors: { unicodeRange: "U+ba27-bb0f" } },
  { uri: Cr, descriptors: { unicodeRange: "U+b93e-ba26" } },
  { uri: Pr, descriptors: { unicodeRange: "U+b855-b93d" } },
  { uri: kr, descriptors: { unicodeRange: "U+b76c-b854" } },
  { uri: a8, descriptors: { unicodeRange: "U+b683-b76b" } },
  { uri: d8, descriptors: { unicodeRange: "U+b59a-b682" } },
  { uri: jr, descriptors: { unicodeRange: "U+b4b1-b599" } },
  { uri: c8, descriptors: { unicodeRange: "U+11fb-11ff,U+b3cd-b4b0" } },
  { uri: f8, descriptors: { unicodeRange: "U+11e6-11fa,U+b2f9-b3cc" } },
  { uri: t8, descriptors: { unicodeRange: "U+11d1-11e5,U+b225-b2f8" } },
  { uri: e8, descriptors: { unicodeRange: "U+11bc-11d0,U+b151-b224" } },
  { uri: o8, descriptors: { unicodeRange: "U+11a7-11bb,U+b07d-b150" } },
  { uri: r8, descriptors: { unicodeRange: "U+1191-11a6,U+afaa-b07c" } },
  { uri: i8, descriptors: { unicodeRange: "U+117c-1190,U+aed6-afa9" } },
  { uri: s8, descriptors: { unicodeRange: "U+1167-117b,U+ae02-aed5" } },
  { uri: b8, descriptors: { unicodeRange: "U+1152-115e,U+1160-1166,U+ad2d-ae01" } },
  { uri: n8, descriptors: { unicodeRange: "U+113d-1151,U+ac59-ad2c" } },
  {
    uri: l8,
    descriptors: {
      unicodeRange:
  ...
  { uri: zr, descriptors: { unicodeRange: "U+49d5-4a77" } },
  {
    uri: ia,
    descriptors: {
      unicodeRange:
  ...
  { uri: sa, descriptors: { unicodeRange: "U+4933-49d4" } },
  { uri: ba, descriptors: { unicodeRange: "U+487a-4932" } },
  { uri: na, descriptors: { unicodeRange: "U+47d2-4879,U+2ce7c,U+2ce88,U+2ce93" } },
  {
    uri: la,
    descriptors: {
      unicodeRange:
  ...
  { uri: Sa, descriptors: { unicodeRange: "U+30a2-30ff,U+3105-312f,U+3131-318e,U+3190-3191" } },
  {
    uri: Wa,
    descriptors: {
      unicodeRange:
  ...
  { uri: Aa, descriptors: { unicodeRange: "U+2e3b,U+2e80-2e99,U+2e9b-2ef3,U+2f00-2f73,U+ffffd" } },
  {
    uri: Ga,
    descriptors: {
      unicodeRange:
  ...
  { uri: Dd, descriptors: { unicodeRange: "U+92ef-933d,U+933f-9369,U+936b-9388" } },
  {
    uri: qd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Er, descriptors: { unicodeRange: "U+4b0b-4ba0" } },
  {
    uri: Yd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Vd, descriptors: { unicodeRange: "U+7d95-7da5,U+7da7-7dad,U+7daf-7e2a" } },
  { uri: wd, descriptors: { unicodeRange: "U+4a78-4b0a" } },
  {
    uri: Od,
    descriptors: {
      unicodeRange:
  ...
  { uri: hd, descriptors: { unicodeRange: "U+99c9-9a53" } },
  {
    uri: Hd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Bd, descriptors: { unicodeRange: "U+4ba1-4c2c" } },
  {
    uri: vd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Zd, descriptors: { unicodeRange: "U+9d1a-9da1" } },
  {
    uri: Sd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Qd, descriptors: { unicodeRange: "U+4cad-4d2f" } },
  {
    uri: Ad,
    descriptors: {
      unicodeRange:
  ...
  { uri: Gd, descriptors: { unicodeRange: "U+9389-938e,U+9390-93c9,U+93cb-93d5,U+93d7-9410" } },
  {
    uri: gd,
    descriptors: {
      unicodeRange:
  ...
  { uri: Cd, descriptors: { unicodeRange: "U+9bd0-9c4a" } },
  { uri: Pd, descriptors: { unicodeRange: "U+4c2d-4cac" } },
  { uri: kd, descriptors: { unicodeRange: "U+4d30-4dab" } },
  { uri: jd, descriptors: { unicodeRange: "U+9411-943d,U+943f-946a,U+946c-9484" } },
  {
    uri: zd,
    descriptors: {
      unicodeRange:
...
var I = class I {
  constructor(e) {
    _(this, "scene");
    _(this, "getSceneFamilies", () => I.getUniqueFamilies(this.scene.getNonDeletedElements()));
    _(this, "onLoaded", (e) => {
      let r = !0;
      for (let t of e) {
        let d = `${t.family}-${t.style}-${t.weight}-${t.unicodeRange}`;
        I.loadedFontsCache.has(d) || (I.loadedFontsCache.add(d), (r = !1));
      }
      if (r) return;
      let a = !1,
        c = this.scene.getNonDeletedElementsMap();
      for (let t of this.scene.getNonDeletedElements())
        if (Ue(t)) {
          (a = !0), f6.delete(t), c7.clearCache(a6(t));
          let d = d7(t, c);
          d && f6.delete(d);
        }
      a && this.scene.triggerUpdate();
    });
    _(this, "loadSceneFonts", async () => {
      let e = this.getSceneFamilies(),
        r = I.getCharsPerFamily(this.scene.getNonDeletedElements());
      return I.loadFontFaces(e, r);
    });
    this.scene = e;
  }
  static get registered() {
    return (
      I._registered
        ? I._initialized ||
          (I._registered = new Map([...I.init().entries(), ...I._registered.entries()]))
        : (I._registered = I.init()),
      I._registered
    );
  }
  get registered() {
  ...
  static async generateFontFaceDeclarations(e) {
    let r = I.getUniqueFamilies(e),
      a = I.getCharsPerFamily(e),
      c = r.find((i) => a7(i).includes(DU));
    if (c) {
      let i = I.getCharacters(a, c);
      if (f7(i)) {
        let s = NU[DU];
        (a[s] = new Set(i)), r.unshift(NU[DU]);
      }
    }
    let t = I.fontFacesStylesGenerator(r, a),
      d = 3,
      f = await new d6(t, d).all();
    return Array.from(new Set(f));
  }
  static async loadFontFaces(e, r) {
    for (let { fontFaces: d, metadata: f } of I.registered.values())
      if (!f.local)
        for (let { fontFace: i } of d) window.document.fonts.has(i) || window.document.fonts.add(i);
    let a = I.fontFacesLoader(e, r),
      c = 10;
    return (await new d6(a, c).all()).flat().filter(Boolean);
  }
  static *fontFacesLoader(e, r) {
    for (let [a, c] of e.entries()) {
      let t = a6({ fontFamily: c, fontSize: 16 }),
        d = I.getCharacters(r, c);
      window.document.fonts.check(t, d) ||
        (yield c6(async () => {
          try {
            let f = await window.document.fonts.load(t, d);
            return [a, f];
          } catch (f) {
            console.error(
              `Failed to load font "${t}" from urls "${I.registered
                .get(c)
                ?.fontFaces.map((i) => i.urls)}"`,
              f
            );
          }
        }));
  ...
  static *fontFacesStylesGenerator(e, r) {
    for (let [a, c] of e.entries()) {
      let { fontFaces: t, metadata: d } = I.registered.get(c) ?? {};
      if (!Array.isArray(t)) {
        console.error(`Couldn't find registered fonts for font-family "${c}"`, I.registered);
        continue;
      }
      if (!d?.local)
        for (let [f, i] of t.entries())
          yield c6(async () => {
            try {
              let s = I.getCharacters(r, c),
                b = await i.toCSS(s);
              return b ? [a * 1e4 + f, b] : void 0;
            } catch (s) {
              console.error(
                `Couldn't transform font-face to css for family "${i.fontFace.family}"`,
                s
              );
            }
          });
  ...
  static register(e, r, ...a) {
    let c = zU[e] ?? NU[e];
    return (
      this.registered.get(c) ||
        this.registered.set(c, {
          metadata: r,
          fontFaces: a.map(({ uri: d, descriptors: f }) => new TU(e, d, f)),
        }),
      this.registered
    );
  }
  static init() {
    let e = { registered: new Map() },
      r = (a, ...c) => {
        let t = zU[a] ?? NU[a],
          d = r6[t] ?? r6[zU.Excalifont];
        I.register.call(e, a, d, ...c);
      };
    return (
      r("Cascadia", ...qe),
      r("Comic Shanns", ...Ve),
  ...
  static getUniqueFamilies(e) {
    return Array.from(e.reduce((r, a) => (Ue(a) && r.add(a.fontFamily), r), new Set()));
  }
  static getCharsPerFamily(e) {
    let r = {};
    for (let a of e)
      if (Ue(a))
        for (let c of a.originalText)
          r[a.fontFamily] || (r[a.fontFamily] = new Set()), r[a.fontFamily].add(c);
    return r;
  }
  static getCharacters(e, r) {
    return e[r] ? Array.from(e[r]).join("") : "";
  }
  static getAllFamilies() {
    return Array.from(I.registered.keys());
  }
};
_(I, "loadedFontsCache", new Set()),
  _(I, "_registered"),
  _(I, "_initialized", !1),
  _(I, "loadElementsFonts", async (e) => {
    let r = I.getUniqueFamilies(e),
      a = I.getCharsPerFamily(e);
    return I.loadFontFaces(r, a);
  });
var bU = I;
import { FRAME_STYLE as y7, throttleRAF as g7 } from "@excalidraw/common";
import { isElementLink as R7 } from "@excalidraw/element";
import { createPlaceholderEmbeddableLabel as $7 } from "@excalidraw/element";
import { getBoundTextElement as u7 } from "@excalidraw/element";
import {
  isEmbeddableElement as x7,
  isIframeLikeElement as de,
  isTextElement as I7,
} from "@excalidraw/element";
import {
  elementOverlapsWithFrame as m7,
  getTargetFrame as l6,
  shouldApplyFrameClip as E6,
} from "@excalidraw/element";
import { renderElement as fU } from "@excalidraw/element";
import { getElementAbsoluteCoords as J7 } from "@excalidraw/element";
import { pointFrom as qU, pointRotateRads as t7 } from "@excalidraw/math";
import { MIME_TYPES as t6 } from "@excalidraw/common";
import { getElementAbsoluteCoords as o7 } from "@excalidraw/element";
import { hitElementBoundingBox as i7 } from "@excalidraw/element";
var s7 = 12,
  ee = document.createElement("img");
ee.src = `data:${t6.svg}, ${encodeURIComponent(
...
re.src = `data:${t6.svg}, ${encodeURIComponent(
...
var ae = ([U, e, r, a], c, t) => {
    let d = s7,
      f = t.zoom.value > 1 ? t.zoom.value : 1,
      i = d / f,
      s = d / f,
      b = d / f,
      R = (U + r) / 2,
      ...
      o = r + E - $,
      l = e - E - b + $,
      [T, g] = t7(qU(o + i / 2, l + s / 2), qU(R, n), c);
    return [T - i / 2, g - s / 2, i, s];
  },
  b7 = (U, e, r, [a, c]) => {
    let t = 4 / r.zoom.value,
      [d, f, i, s] = o7(U, e),
      [b, R, n, $] = ae([d, f, i, s], U.angle, r);
    return a > b - t && a < b + t + n && c > R - t && c < R + $ + t;
  },
  AE = (U, e, r, [a, c], t) =>
    !U.link || r.selectedElementIds[U.id]
      ? !1
      : !t && r.viewModeEnabled && i7(qU(a, c), U, e)
      ? !0
      : b7(U, e, r, qU(a, c));
import { THEME as n7, THEME_FILTER as l7 } from "@excalidraw/common";
import { FIXED_BINDING_DISTANCE as C } from "@excalidraw/element";
import { getDiamondPoints as o6 } from "@excalidraw/element";
import { elementCenterPoint as i6, getCornerRadius as nU } from "@excalidraw/element";
import {
  curve as S,
  curveCatmullRomCubicApproxPoints as E7,
  curveCatmullRomQuadraticApproxPoints as p7,
  curveOffsetPoints as W,
  offsetPointsForQuadraticBezier as Q,
  pointFrom as y,
  pointRotateRads as s6,
} from "@excalidraw/math";
var ep = (U, e, r, a, c, t = !0) => {
    U.beginPath(), U.arc(e, r, a, 0, Math.PI * 2), t && U.fill(), c && U.stroke();
  },
  b6 = (U, e) => [U.width / e, U.height / e],
  n6 = ({
    canvas: U,
    scale: e,
    normalizedWidth: r,
    ...
    return (
      ...
      f.scale(e, e),
      t && c === n7.DARK && (f.filter = l7),
      typeof d == "string"
        ? ((d === "transparent" || d.length === 5 || d.length === 9 || /(hsla|rgba)\(/.test(d)) &&
            f.clearRect(0, 0, r, a),
          f.save(),
          (f.fillStyle = d),
          f.fillRect(0, 0, r, a),
...
function A(U, e, r = 0.5) {
  let a = p7(e, r);
  if (a)
    for (let c = 0; c < a.length - 1; c++) {
      let [[t, d], [f, i]] = a[c];
      U.quadraticCurveTo(t, d, f, i);
    }
}
function G(U, e, r = 0.5) {
  let a = E7(e, r);
  if (a)
    for (let c = 0; c < a.length; c++) {
      let [[t, d], [f, i], [s, b]] = a[c];
      U.bezierCurveTo(t, d, f, i, s, b);
    }
}
var rp = (U, e, r, a) => {
    let [c, t] = s6(y(e.x, e.y), i6(e, r), e.angle);
    U.save(), U.translate(c, t), U.rotate(e.angle);
    let d = nU(Math.min(e.width, e.height), e);
    d === 0 && (d = 0.01), U.beginPath();
    {
      let f = Q(y(0, 0 + d), y(0, 0), y(0 + d, 0), a),
        i = Q(y(e.width - d, 0), y(e.width, 0), y(e.width, d), a),
        s = Q(y(e.width, e.height - d), y(e.width, e.height), y(e.width - d, e.height), a),
        b = Q(y(d, e.height), y(0, e.height), y(0, e.height - d), a);
      U.moveTo(f[f.length - 1][0], f[f.length - 1][1]),
        U.lineTo(i[0][0], i[0][1]),
        A(U, i),
        U.lineTo(s[0][0], s[0][1]),
        A(U, s),
        U.lineTo(b[0][0], b[0][1]),
        A(U, b),
        U.lineTo(f[0][0], f[0][1]),
        A(U, f);
    }
    {
      let f = Q(y(0 + d, 0), y(0, 0), y(0, 0 + d), -C),
        i = Q(y(e.width, d), y(e.width, 0), y(e.width - d, 0), -C),
        s = Q(y(e.width - d, e.height), y(e.width, e.height), y(e.width, e.height - d), -C),
        b = Q(y(0, e.height - d), y(0, e.height), y(d, e.height), -C);
      U.moveTo(f[f.length - 1][0], f[f.length - 1][1]),
        U.lineTo(b[0][0], b[0][1]),
        A(U, b),
        U.lineTo(s[0][0], s[0][1]),
  ...
  ap = (U, e, r, a, c, t) => {
    U.beginPath(), U.ellipse(a, c, e / 2, r / 2, t, 0, Math.PI * 2), U.stroke();
  },
  dp = (U, e, r, a, c, t, d, f, i = !1, s = 0) => {
    U.save(),
      U.translate(t, d),
      U.rotate(f),
      i && U.fillRect(e - t, r - d, a, c),
      s && U.roundRect
        ? (U.beginPath(), U.roundRect(e - t, r - d, a, c, s), U.stroke(), U.closePath())
        : U.strokeRect(e - t, r - d, a, c),
      U.restore();
  },
  cp = (U, e, r, a) => {
    let [c, t] = s6(y(r.x, r.y), i6(r, a), r.angle);
    U.save(), U.translate(c, t), U.rotate(r.angle);
    {
      U.beginPath();
      let [d, f, i, s, b, R, n, $] = o6(r),
        E = r.roundness ? nU(Math.abs(d - n), r) : (d - n) * 0.01,
        o = r.roundness ? nU(Math.abs(s - f), r) : (s - f) * 0.01,
        l = W(S(y(d - E, f + o), y(d, f), y(d, f), y(d + E, f + o)), e),
        T = W(S(y(i - E, s - o), y(i, s), y(i, s), y(i - E, s + o)), e),
        g = W(S(y(b + E, R - o), y(b, R), y(b, R), y(b - E, R - o)), e),
        p = W(S(y(n + E, $ + o), y(n, $), y(n, $), y(n + E, $ - o)), e);
      U.moveTo(l[l.length - 1][0], l[l.length - 1][1]),
        U.lineTo(T[1][0], T[1][1]),
        G(U, T),
        U.lineTo(g[1][0], g[1][1]),
        G(U, g),
        U.lineTo(p[1][0], p[1][1]),
        G(U, p),
        U.lineTo(l[1][0], l[1][1]),
        G(U, l);
    }
    {
      let [d, f, i, s, b, R, n, $] = o6(r),
        E = r.roundness ? nU(Math.abs(d - n), r) : (d - n) * 0.01,
        o = r.roundness ? nU(Math.abs(s - f), r) : (s - f) * 0.01,
        l = W(S(y(d + E, f + o), y(d, f), y(d, f), y(d - E, f + o)), -C),
        T = W(S(y(i - E, s + o), y(i, s), y(i, s), y(i - E, s - o)), -C),
        g = W(S(y(b - E, R - o), y(b, R), y(b, R), y(b + E, R - o)), -C),
        p = W(S(y(n + E, $ - o), y(n, $), y(n, $), y(n + E, $ + o)), -C);
      U.moveTo(l[l.length - 1][0], l[l.length - 1][1]),
        U.lineTo(p[1][0], p[1][1]),
        G(U, p),
        U.lineTo(g[1][0], g[1][1]),
...
var FU = { Bold: "#dddddd", Regular: "#e5e5e5" },
  K7 = (U, e, r, a, c, t, d, f) => {
    let i = (a % e) - e,
      s = (c % e) - e,
      b = e * t.value,
      R = 1 / t.value;
    U.save(), t.value === 1 && U.translate(i % 1 ? 0 : 0.5, s % 1 ? 0 : 0.5);
    for (let n = i; n < i + d + e * 2; n += e) {
      let $ = r > 1 && Math.round(n - a) % (r * e) === 0;
      if (!$ && b < 10) continue;
      let E = Math.min(1 / t.value, $ ? 4 : 1);
      U.lineWidth = E;
      let o = [E * 3, R + (E + R)];
      U.beginPath(),
        U.setLineDash($ ? [] : o),
        (U.strokeStyle = $ ? FU.Bold : FU.Regular),
        U.moveTo(n, s - e),
        U.lineTo(n, Math.ceil(s + f + e * 2)),
        U.stroke();
    }
    for (let n = s; n < s + f + e * 2; n += e) {
      let $ = r > 1 && Math.round(n - c) % (r * e) === 0;
      if (!$ && b < 10) continue;
      let E = Math.min(1 / t.value, $ ? 4 : 1);
      U.lineWidth = E;
      let o = [E * 3, R + (E + R)];
      U.beginPath(),
        ...
        (U.strokeStyle = $ ? FU.Bold : FU.Regular),
        U.moveTo(i - e, n),
        U.lineTo(Math.ceil(i + d + e * 2), n),
        U.stroke();
    }
    U.restore();
  },
  p6 = (U, e, r, a) => {
    e.translate(U.x + a.scrollX, U.y + a.scrollY),
      e.beginPath(),
      e.roundRect
        ? e.roundRect(0, 0, U.width, U.height, y7.radius / a.zoom.value)
        : e.rect(0, 0, U.width, U.height),
      e.clip(),
      e.translate(-(U.x + a.scrollX), -(U.y + a.scrollY));
  },
  y6 = { regularLink: null, elementLink: null },
  g6 = (U, e, r, a) => {
    if (U.link && !r.selectedElementIds[U.id]) {
      let [c, t, d, f] = J7(U, a),
        [i, s, b, R] = ae([c, t, d, f], U.angle, r),
        n = i + b / 2,
        $ = s + R / 2;
      e.save(), e.translate(r.scrollX + n, r.scrollY + $), e.rotate(U.angle);
      let E = R7(U.link) ? "elementLink" : "regularLink",
        o = y6[E];
      if (!o || o.zoom !== r.zoom.value) {
        (o = Object.assign(document.createElement("canvas"), { zoom: r.zoom.value })),
          (o.width = b * window.devicePixelRatio * r.zoom.value),
          (o.height = R * window.devicePixelRatio * r.zoom.value),
          (y6[E] = o);
        let l = o.getContext("2d");
        l.scale(window.devicePixelRatio * r.zoom.value, window.devicePixelRatio * r.zoom.value),
          (l.fillStyle = r.viewBackgroundColor || "#fff"),
          l.fillRect(0, 0, b, R),
          E === "elementLink" ? l.drawImage(re, 0, 0, b, R) : l.drawImage(ee, 0, 0, b, R),
          l.restore();
      }
      e.drawImage(o, i - n, s - $, b, R), e.restore();
    }
  },
  R6 = ({
    canvas: U,
    ...
    if (U === null) return;
    let { renderGrid: i = !0, isExporting: s } = f,
      [b, R] = b6(U, t),
      n = n6({
        canvas: U,
        scale: t,
    ...
    n.scale(d.zoom.value, d.zoom.value),
      i &&
        K7(
          n,
          d.gridSize,
    ...
    c.forEach((o) => {
      o.groupIds.length > 0 &&
        d.frameToHighlight &&
        d.selectedElementIds[o.id] &&
        (m7(o, d.frameToHighlight, r) || o.groupIds.find((l) => $.has(l))) &&
        o.groupIds.forEach((l) => $.add(l));
    });
    let E = new Map();
    c
      .filter((o) => !de(o))
      .forEach((o) => {
        try {
          let l = o.frameId || d.frameToHighlight?.id;
          if (I7(o) && o.containerId && r.has(o.containerId)) return;
          if ((n.save(), l && d.frameRendering.enabled && d.frameRendering.clip)) {
            let g = l6(o, r, d);
            g && E6(o, g, d, r, E) && p6(g, n, f, d), fU(o, r, a, e, n, f, d);
          } else fU(o, r, a, e, n, f, d);
          let T = u7(o, r);
          T && fU(T, r, a, e, n, f, d), n.restore(), s || g6(o, n, d, r);
        } catch (l) {
          console.error(l, o.id, o.x, o.y, o.width, o.height);
        }
      }),
      c
        .filter((o) => de(o))
          ...
          try {
            let l = () => {
              if (
                (fU(o, r, a, e, n, f, d),
                de(o) &&
                  (s || (x7(o) && f.embedsValidationStatus.get(o.id) !== !0)) &&
                  o.width &&
                  o.height)
              ) {
                let g = $7(o);
            ...
            if (
              (o.frameId || d.frameToHighlight?.id) &&
              d.frameRendering.enabled &&
              d.frameRendering.clip
            ) {
              n.save();
              let g = l6(o, r, d);
              g && E6(o, g, d, r, E) && p6(g, n, f, d), l(), n.restore();
            } else l();
          } catch (l) {
            console.error(l);
          }
        }),
      f.pendingFlowchartNodes?.forEach((o) => {
        try {
          fU(o, r, a, e, n, f, d);
        } catch (l) {
          console.error(l);
...
import {
  FRAME_STYLE as YU,
  MAX_DECIMALS_FOR_SVG_EXPORT as lU,
  MIME_TYPES as N7,
  SVG_NS as Y,
  getFontFamilyString as D7,
  isRTL as q7,
  isTestEnv as F7,
  getVerticalOffset as Y7,
} from "@excalidraw/common";
import { normalizeLink as u6, toValidURL as _7 } from "@excalidraw/common";
import { hashString as L7 } from "@excalidraw/element";
import { getUncroppedWidthAndHeight as x6 } from "@excalidraw/element";
import { createPlaceholderEmbeddableLabel as V7, getEmbedLink as w7 } from "@excalidraw/element";
import { LinearElementEditor as I6 } from "@excalidraw/element";
import { getBoundTextElement as T6, getContainerElement as O7 } from "@excalidraw/element";
import { getLineHeightInPx as X7 } from "@excalidraw/element";
import {
  isArrowElement as h7,
  isIframeLikeElement as m6,
  isInitializedImageElement as H7,
  isTextElement as ce,
} from "@excalidraw/element";
import { getContainingFrame as N6 } from "@excalidraw/element";
import { getCornerRadius as J6, isPathALoop as M7 } from "@excalidraw/element";
import { ShapeCache as _U } from "@excalidraw/element";
import { getFreeDrawSvgPath as B7, IMAGE_INVERT_FILTER as v7 } from "@excalidraw/element";
import { getElementAbsoluteCoords as K6 } from "@excalidraw/element";
var EU = (U, e, r) => {
    if (typeof r > "u") return U.draw(e);
    let a = { sets: e.sets, shape: e.shape, options: { ...e.options, fixedDecimalPlaceDigits: r } };
    return U.draw(a);
  },
  pU = (U, e, r, a, c) => {
    ...
    let t = N6(U, c);
    if (t) {
      let d = e.ownerDocument.createElementNS(Y, "g");
      return (
        d.setAttributeNS(Y, "clip-path", `url(#${t.id})`), r.forEach((f) => d.appendChild(f)), d
      );
    }
    return null;
  },
  LU = (U, e, r, a, c, t, d, f) => {
    ...
    if (ce(U)) {
      ...
      if (h7(p)) {
        let [u, K, V, q] = K6(p, e),
          N = I6.getBoundTextElementPosition(p, U, e);
        ($ = (V - u) / 2 - (N.x - u)),
          (E = (q - K) / 2 - (N.y - K)),
          (t = t + N.x - U.x),
          (d = d + N.y - U.y);
    ...
    let o = (180 * U.angle) / Math.PI,
      l = a;
    if (U.link) {
      let p = a.ownerDocument.createElementNS(Y, "a");
      p.setAttribute("href", u6(U.link)), l.appendChild(p), (l = p);
    }
    let T = (p, u) => {
        F7() && p.setAttribute("data-id", u.id), l.appendChild(p);
      },
      g = ((N6(U, e)?.opacity ?? 100) * U.opacity) / 1e4;
    switch (U.type) {
      case "selection":
      ...
      case "diamond":
      case "ellipse": {
        let p = _U.generateElementShape(U, null),
          u = EU(r, p, lU);
        g !== 1 &&
          (u.setAttribute("stroke-opacity", `${g}`), u.setAttribute("fill-opacity", `${g}`)),
          u.setAttribute("stroke-linecap", "round"),
          u.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`);
        let K = pU(U, l, [u], f.frameRendering, e);
        T(K || u, U);
        break;
      }
      case "iframe":
      case "embeddable": {
        let p = _U.generateElementShape(U, f),
          u = EU(r, p, lU),
          K = U.opacity / 100;
        K !== 1 &&
          (u.setAttribute("stroke-opacity", `${K}`), u.setAttribute("fill-opacity", `${K}`)),
          u.setAttribute("stroke-linecap", "round"),
          u.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`),
          T(u, U);
        let V = V7(U);
        LU(V, e, r, l, c, V.x + i.x - U.x, V.y + i.y - U.y, f);
        let q = EU(r, p, lU);
        for (
          q.setAttribute("stroke-linecap", "round"),
            q.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`);
          q.firstChild;

        )
          q.removeChild(q.firstChild);
        let N = J6(Math.min(U.width, U.height), U),
          F = w7(_7(U.link || ""));
        if (f.renderEmbeddables === !1 || F?.type === "document") {
          let D = a.ownerDocument.createElementNS(Y, "a");
          D.setAttribute("href", u6(U.link || "")),
            D.setAttribute("target", "_blank"),
            D.setAttribute("rel", "noopener noreferrer"),
            (D.style.borderRadius = `${N}px`),
            q.appendChild(D);
        } else {
          let D = a.ownerDocument.createElementNS(Y, "foreignObject");
          (D.style.width = `${U.width}px`),
            (D.style.height = `${U.height}px`),
            (D.style.border = "none");
          let m = D.ownerDocument.createElementNS(Y, "div");
          m.setAttribute("xmlns", "http://www.w3.org/1999/xhtml"),
            (m.style.width = "100%"),
            (m.style.height = "100%");
          let x = m.ownerDocument.createElement("iframe");
          (x.src = F?.link ?? ""),
            (x.style.width = "100%"),
            (x.style.height = "100%"),
            (x.style.border = "none"),
            (x.style.borderRadius = `${N}px`),
            (x.style.top = "0"),
            (x.style.left = "0"),
            (x.allowFullscreen = !0),
            m.appendChild(x),
      ...
      case "arrow": {
        let p = T6(U, e),
          u = a.ownerDocument.createElementNS(Y, "mask");
        if (p) {
          u.setAttribute("id", `mask-${U.id}`);
          let N = a.ownerDocument.createElementNS(Y, "rect");
          (t = t || 0),
            (d = d || 0),
            N.setAttribute("x", "0"),
            N.setAttribute("y", "0"),
            N.setAttribute("fill", "#fff"),
            N.setAttribute("width", `${U.width + 100 + t}`),
            N.setAttribute("height", `${U.height + 100 + d}`),
            u.appendChild(N);
          let F = a.ownerDocument.createElementNS(Y, "rect"),
            D = I6.getBoundTextElementPosition(U, p, e),
            m = t + D.x - U.x,
            x = d + D.y - U.y;
          F.setAttribute("x", m.toString()),
            F.setAttribute("y", x.toString()),
            F.setAttribute("fill", "#000"),
            F.setAttribute("width", `${p.width}`),
            F.setAttribute("height", `${p.height}`),
            F.setAttribute("opacity", "1"),
            u.appendChild(F);
        }
        let K = a.ownerDocument.createElementNS(Y, "g");
        p && K.setAttribute("mask", `url(#mask-${U.id})`),
          K.setAttribute("stroke-linecap", "round"),
          _U.generateElementShape(U, f).forEach((N) => {
            let F = EU(r, N, lU);
            g !== 1 &&
              (F.setAttribute("stroke-opacity", `${g}`), F.setAttribute("fill-opacity", `${g}`)),
              F.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`),
              U.type === "line" &&
                M7(U.points) &&
        ...
        let q = pU(U, l, [K, u], f.frameRendering, e);
        q ? (T(q, U), l.appendChild(q)) : (T(K, U), l.append(u));
        break;
      }
      case "freedraw": {
        let p = _U.generateElementShape(U, f),
          u = p ? EU(r, p, lU) : a.ownerDocument.createElementNS(Y, "g");
        g !== 1 &&
          (u.setAttribute("stroke-opacity", `${g}`), u.setAttribute("fill-opacity", `${g}`)),
          u.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`),
          u.setAttribute("stroke", "none");
        let K = a.ownerDocument.createElementNS(Y, "path");
        K.setAttribute("fill", U.strokeColor), K.setAttribute("d", B7(U)), u.appendChild(K);
        let V = pU(U, l, [u], f.frameRendering, e);
        T(V || u, U);
        break;
      }
      case "image": {
        let p = Math.round(U.width),
          u = Math.round(U.height),
          K = H7(U) && c[U.fileId];
        if (K) {
          let { reuseImages: V = !0 } = f,
            q = `image-${K.id}`,
            N = U.width,
            F = U.height;
          U.crop &&
            (({ width: N, height: F } = x6(U)), (q = `image-crop-${K.id}-${L7(`${N}x${F}`)}`)),
            V || (q = `image-${U.id}`);
          let D = a.querySelector(`#${q}`);
          if (!D) {
            (D = a.ownerDocument.createElementNS(Y, "symbol")), (D.id = q);
            let L = a.ownerDocument.createElementNS(Y, "image");
            L.setAttribute("href", K.dataURL),
              L.setAttribute("preserveAspectRatio", "none"),
              U.crop || !V
                ? (L.setAttribute("width", `${N}`), L.setAttribute("height", `${F}`))
                : (L.setAttribute("width", "100%"), L.setAttribute("height", "100%")),
              D.appendChild(L),
              (l.querySelector("defs") || l).prepend(D);
          }
          let m = a.ownerDocument.createElementNS(Y, "use");
          m.setAttribute("href", `#${q}`),
            f.exportWithDarkMode && K.mimeType !== N7.svg && m.setAttribute("filter", v7);
          let x = 0,
            X = 0;
          if (U.crop) {
            let { width: L, height: w } = x6(U);
            (x = U.crop.x / (U.crop.naturalWidth / L)), (X = U.crop.y / (U.crop.naturalHeight / w));
          }
          let UU = $ + x,
            eU = E + X;
          m.setAttribute("width", `${p + x}`),
            m.setAttribute("height", `${u + X}`),
            m.setAttribute("opacity", `${g}`),
            (U.scale[0] !== 1 || U.scale[1] !== 1) &&
              m.setAttribute(
                "transform",
                `translate(${UU} ${eU}) scale(${U.scale[0]} ${U.scale[1]}) translate(${-UU} ${-eU})`
              );
          let v = a.ownerDocument.createElementNS(Y, "g");
          if (U.crop) {
            let L = a.ownerDocument.createElementNS(Y, "mask");
            L.setAttribute("id", `mask-image-crop-${U.id}`), L.setAttribute("fill", "#fff");
            let w = a.ownerDocument.createElementNS(Y, "rect");
            w.setAttribute("x", `${x}`),
              w.setAttribute("y", `${X}`),
              w.setAttribute("width", `${p}`),
              w.setAttribute("height", `${u}`),
              L.appendChild(w),
              l.appendChild(L),
              v.setAttribute("mask", `url(#${L.id})`);
          }
          if (
            (v.appendChild(m),
            v.setAttribute("transform", `translate(${t - x} ${d - X}) rotate(${o} ${UU} ${eU})`),
            U.roundness)
          ) {
            let L = a.ownerDocument.createElementNS(Y, "clipPath");
            L.id = `image-clipPath-${U.id}`;
            let w = a.ownerDocument.createElementNS(Y, "rect"),
              h = J6(Math.min(U.width, U.height), U);
            w.setAttribute("width", `${U.width}`),
              w.setAttribute("height", `${U.height}`),
              w.setAttribute("rx", `${h}`),
              w.setAttribute("ry", `${h}`),
              L.appendChild(w),
              T(L, U),
              v.setAttributeNS(Y, "clip-path", `url(#${L.id})`);
          }
          let HU = pU(U, l, [v], f.frameRendering, e);
          T(HU || v, U);
        }
        break;
      }
      case "frame":
      case "magicframe": {
        if (f.frameRendering.enabled && f.frameRendering.outline) {
          let p = document.createElementNS(Y, "rect");
          p.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`),
            p.setAttribute("width", `${U.width}px`),
            p.setAttribute("height", `${U.height}px`),
            p.setAttribute("rx", YU.radius.toString()),
            p.setAttribute("ry", YU.radius.toString()),
            p.setAttribute("fill", "none"),
            p.setAttribute("stroke", YU.strokeColor),
            p.setAttribute("stroke-width", YU.strokeWidth.toString()),
            T(p, U);
        }
        break;
      }
      default:
        if (ce(U)) {
          let p = a.ownerDocument.createElementNS(Y, "g");
          g !== 1 &&
            (p.setAttribute("stroke-opacity", `${g}`), p.setAttribute("fill-opacity", `${g}`)),
            p.setAttribute("transform", `translate(${t || 0} ${d || 0}) rotate(${o} ${$} ${E})`);
          let u = U.text.replace(
              /\r\n?/g,
              `
`
            ).split(`
`),
            K = X7(U.fontSize, U.lineHeight),
            V = U.textAlign === "center" ? U.width / 2 : U.textAlign === "right" ? U.width : 0,
            q = Y7(U.fontFamily, U.fontSize, K),
            N = q7(U.text) ? "rtl" : "ltr",
            F =
              U.textAlign === "center"
                ? "middle"
                : U.textAlign === "right" || N === "rtl"
                ? "end"
                : "start";
          for (let m = 0; m < u.length; m++) {
            let x = a.ownerDocument.createElementNS(Y, "text");
            (x.textContent = u[m]),
              x.setAttribute("x", `${V}`),
              x.setAttribute("y", `${m * K + q}`),
              x.setAttribute("font-family", D7(U)),
              x.setAttribute("font-size", `${U.fontSize}px`),
              x.setAttribute("fill", U.strokeColor),
              x.setAttribute("text-anchor", F),
              x.setAttribute("style", "white-space: pre;"),
              x.setAttribute("direction", N),
              x.setAttribute("dominant-baseline", "alphabetic"),
              p.appendChild(x);
          }
          let D = pU(U, l, [p], f.frameRendering, e);
          T(D || p, U);
        } else throw new Error(`Unimplemented type ${U.type}`);
    }
  },
  D6 = (U, e, r, a, c, t) => {
    a &&
      (U.filter((d) => !m6(d)).forEach((d) => {
        if (!d.isDeleted) {
          if (ce(d) && d.containerId && e.has(d.containerId)) return;
          try {
            LU(d, e, r, a, c, d.x + t.offsetX, d.y + t.offsetY, t);
            let f = T6(d, e);
            f && LU(f, e, r, a, c, f.x + t.offsetX, f.y + t.offsetY, t);
          } catch (f) {
            console.error(f);
          }
        }
...
var ao = (U, e) => {
    if (U.width <= e) return U;
    let a = document.createElement("canvas").getContext("2d");
    a.font = Q7({ fontFamily: U.fontFamily, fontSize: U.fontSize });
    let c = U.text;
    if (a.measureText(c).width > e)
      for (let d = c.length; d > 0; d--) {
        let f = `${c.slice(0, d)}...`;
        if (a.measureText(f).width <= e) {
          c = f;
          break;
        }
      }
    return k7(U, { text: c, width: e });
  },
  co = (U, e) => {
    let r = [];
    for (let a of U) {
      if (V6(a)) {
        let c = ro({
          x: a.x,
          y: a.y - j.nameOffsetY,
          fontFamily: Z7.Helvetica,
          fontSize: j.nameFontSize,
          lineHeight: j.nameLineHeight,
          strokeColor: e.exportWithDarkMode ? j.nameColorDarkTheme : j.nameColorLightTheme,
          text: Uo(a),
        });
        (c.y -= c.height), (c = ao(c, a.width)), r.push(c);
      }
      r.push(a);
    }
    return r;
  ...
  O6 = (U, e) => (
    (e = e || oU().frameRendering),
    {
      enabled: U ? !0 : e.enabled,
      outline: U ? !1 : e.outline,
      name: U ? !1 : e.name,
      clip: U ? !0 : e.clip,
    }
  ),
  X6 = ({ elements: U, exportingFrame: e, frameRendering: r, exportWithDarkMode: a }) => {
    let c;
    return (
      e ? (c = j7(U, e)) : r.enabled && r.name ? (c = co(U, { exportWithDarkMode: a })) : (c = U), c
    );
  },
  kp = async (
    U,
    e,
    r,
    { exportBackground: a, exportPadding: c = _6, viewBackgroundColor: t, exportingFrame: d },
    f = (s, b) => {
      let R = document.createElement("canvas");
      return (
        (R.width = s * e.exportScale),
        (R.height = b * e.exportScale),
        { canvas: R, scale: e.exportScale }
      );
    },
    i = async () => {
      await bU.loadElementsFonts(U);
    }
  ) => {
    await i();
    let s = O6(d ?? null, e.frameRendering ?? null);
    d && (s.clip = !1);
    let b = X6({
      elements: U,
      exportingFrame: d,
      exportWithDarkMode: e.exportWithDarkMode,
      frameRendering: s,
    });
    d && (c = 0);
    let [R, n, $, E] = te(d ? [d] : w6(b), c),
      { canvas: o, scale: l = 1 } = f($, E),
      T = oU(),
      { imageCache: g } = await P7({
        imageCache: new Map(),
        fileIds: C7(b).map((p) => p.fileId),
        files: r,
      });
    return (
      $6({
        ...
        rc: Y6.canvas(o),
        elementsMap: fe(VU(b)),
        allElementsMap: fe(VU(eo(U))),
        visibleElements: b,
        scale: l,
        appState: {
          ...e,
          frameRendering: s,
          viewBackgroundColor: a ? t : null,
          scrollX: -R + c,
          scrollY: -n + c,
          zoom: T.zoom,
          shouldCacheIgnoreZoom: !1,
          theme: e.exportWithDarkMode ? q6.DARK : q6.LIGHT,
        },
        renderConfig: {
          canvasBackgroundColor: t,
          imageCache: g,
          renderGrid: !1,
          isExporting: !0,
  ...
  yU = (U) => document.createComment(` ${U} `),
  jp = async (U, e, r, a) => {
    let c = O6(a?.exportingFrame ?? null, e.frameRendering ?? null),
      {
        exportPadding: t = _6,
        exportWithDarkMode: d = !1,
        viewBackgroundColor: f,
        exportScale: i = 1,
        exportEmbedScene: s,
      } = e,
      { exportingFrame: b = null } = a || {},
      R = X6({ elements: U, exportingFrame: b, exportWithDarkMode: d, frameRendering: c });
    b && (t = 0);
    let [n, $, E, o] = te(b ? [b] : w6(R), t),
      l = -n + t,
      T = -$ + t,
      g = document.createElementNS(P, "svg");
    g.setAttribute("version", "1.1"),
      g.setAttribute("xmlns", P),
      g.setAttribute("viewBox", `0 0 ${E} ${o}`),
      g.setAttribute("width", `${E * i}`),
      g.setAttribute("height", `${o * i}`),
      d && g.setAttribute("filter", S7);
    let p = g.ownerDocument.createElementNS(P, "defs"),
      u = g.ownerDocument.createElementNS(P, "metadata");
    if ((g.appendChild(yU("svg-source:excalidraw")), g.appendChild(u), g.appendChild(p), s))
      try {
        fo({ metadataElement: u, payload: QU(U, e, r || {}, "local") });
      } catch (m) {
        console.error(m);
      }
    let K = z7(U);
    if (K.length) {
      let m = VU(U);
      for (let x of K) {
        let X = g.ownerDocument.createElementNS(P, "clipPath");
        X.setAttribute("id", x.id);
        let [UU, eU, v, HU] = G7(x, m),
          L = (v - UU) / 2 - (x.x - UU),
          w = (HU - eU) / 2 - (x.y - eU),
          h = g.ownerDocument.createElementNS(P, "rect");
        h.setAttribute(
          "transform",
          `translate(${x.x + l} ${x.y + T}) rotate(${x.angle} ${L} ${w})`
        ),
          h.setAttribute("width", `${x.width}`),
          h.setAttribute("height", `${x.height}`),
          b || (h.setAttribute("rx", `${j.radius}`), h.setAttribute("ry", `${j.radius}`)),
          X.appendChild(h),
          p.appendChild(X);
      }
    }
    let V = a?.skipInliningFonts ? [] : await bU.generateFontFaceDeclarations(U),
      q = `
      `,
      N = g.ownerDocument.createElementNS(P, "style");
    if (
      (N.classList.add("style-fonts"),
      N.appendChild(document.createTextNode(`${q}${V.join(q)}`)),
      p.appendChild(N),
      e.exportBackground && f)
    ) {
      let m = g.ownerDocument.createElementNS(P, "rect");
      m.setAttribute("x", "0"),
        m.setAttribute("y", "0"),
        m.setAttribute("width", `${E}`),
        m.setAttribute("height", `${o}`),
        m.setAttribute("fill", f),
        g.appendChild(m);
    }
    let F = Y6.svg(g),
      D = a?.renderEmbeddables ?? !1;
    return (
      D6(R, fe(VU(R)), F, g, r || {}, {
        offsetX: l,
        offsetY: T,
        isExporting: !0,
        exportWithDarkMode: d,
        renderEmbeddables: D,
        frameRendering: c,
        canvasBackgroundColor: f,
        embedsValidationStatus: D
          ? new Map(R.filter((m) => V6(m)).map((m) => [m.id, !0]))
          : new Map(),
        reuseImages: a?.reuseImages ?? !0,
      }),
      g
    );
  },
  fo = ({ payload: U, metadataElement: e }) => {
    let r = uU(JSON.stringify(IU({ text: U })), !0);
    e.appendChild(yU(`payload-type:${L6.excalidraw}`)),
      e.appendChild(yU("payload-version:2")),
      e.appendChild(yU("payload-start")),
      e.appendChild(document.createTextNode(r)),
      e.appendChild(yU("payload-end"));
  },
  h6 = ({ svg: U }) => {
    if (U.includes(`payload-type:${L6.excalidraw}`)) {
      let e = U.match(/<!-- payload-start -->\s*(.+?)\s*<!-- payload-end -->/);
      if (!e) throw new Error("INVALID");
      let c = (U.match(/<!-- payload-version:(\d+) -->/)?.[1] || "1") !== "1";
      try {
        let t = xU(e[1], c),
          d = JSON.parse(t);
        if (!("encoded" in d)) {
          if ("type" in d && d.type === W7.excalidraw) return t;
          throw new Error("FAILED");
        }
        return mU(d);
      } catch (t) {
        throw (console.error(t), new Error("FAILED"));
      }
    }
  ...
  zp = (U, e, r) => {
    let [, , a, c] = te(U, e).map((t) => Math.trunc(t * r));
    return [a, c];
  };
import { isFiniteNumber as oe, pointFrom as B } from "@excalidraw/math";
import {
  DEFAULT_FONT_FAMILY as to,
  DEFAULT_TEXT_ALIGN as oo,
  DEFAULT_VERTICAL_ALIGN as io,
  FONT_FAMILY as H6,
  ROUNDNESS as M6,
  DEFAULT_SIDEBAR as so,
  DEFAULT_ELEMENT_PROPS as z,
  DEFAULT_GRID_SIZE as bo,
  DEFAULT_GRID_STEP as no,
  randomId as wU,
  getUpdatedTimestamp as lo,
  updateActiveTool as Eo,
  arrayToMap as B6,
  getSizeFromPoints as v6,
  normalizeLink as po,
  getLineHeight as yo,
} from "@excalidraw/common";
import { getNonDeletedElements as go, isValidPolygon as Ro } from "@excalidraw/element";
import { normalizeFixedPoint as $o } from "@excalidraw/element";
import { updateElbowArrowPoints as uo, validateElbowPoints as xo } from "@excalidraw/element";
import { LinearElementEditor as Z6 } from "@excalidraw/element";
import { bumpVersion as Q6 } from "@excalidraw/element";
import { getContainerElement as Io } from "@excalidraw/element";
import { detectLineHeight as mo } from "@excalidraw/element";
import {
  isArrowBoundToElement as Jo,
  isArrowElement as ie,
  isElbowArrow as OU,
  isFixedPointBinding as Ko,
  isLinearElement as To,
  isLineElement as No,
  isTextElement as se,
  isUsingAdaptiveRadius as Do,
} from "@excalidraw/element";
import { syncInvalidIndices as qo } from "@excalidraw/element";
import { refreshTextDimensions as Fo } from "@excalidraw/element";
import { getNormalizedDimensions as Yo } from "@excalidraw/element";
import { isInvisiblySmallElement as _o } from "@excalidraw/element";
var Lo = {
    selection: !0,
    lasso: !0,
    text: !0,
  ...
  Vo = (U) => (Object.keys(H6).includes(U) ? H6[U] : to),
  tU = (U, e) => {
    if (!e) return null;
    let r = e.focus || 0;
    return OU(U)
      ? Ko(e)
        ? { ...e, focus: r, fixedPoint: $o(e.fixedPoint ?? [0, 0]) }
        : null
      : { ...e, focus: r };
  },
  k = (U, e) => {
    let r = {
      type: e.type || U.type,
      version: U.version || 1,
      versionNonce: U.versionNonce ?? 0,
      index: U.index ?? null,
      isDeleted: U.isDeleted ?? !1,
      id: U.id || wU(),
      fillStyle: U.fillStyle || z.fillStyle,
      strokeWidth: U.strokeWidth || z.strokeWidth,
      strokeStyle: U.strokeStyle ?? z.strokeStyle,
      roughness: U.roughness ?? z.roughness,
      opacity: U.opacity == null ? z.opacity : U.opacity,
      angle: U.angle || 0,
      x: e.x ?? U.x ?? 0,
      y: e.y ?? U.y ?? 0,
      strokeColor: U.strokeColor || z.strokeColor,
      backgroundColor: U.backgroundColor || z.backgroundColor,
      width: U.width || 0,
      height: U.height || 0,
      seed: U.seed ?? 1,
      groupIds: U.groupIds ?? [],
      ...
      roundness: U.roundness
        ? U.roundness
        : U.strokeSharpness === "round"
        ? { type: Do(U.type) ? M6.LEGACY : M6.PROPORTIONAL_RADIUS }
        : null,
      boundElements: U.boundElementIds
        ? U.boundElementIds.map((c) => ({ type: "arrow", id: c }))
        : U.boundElements ?? [],
      updated: U.updated ?? lo(),
      link: U.link ? po(U.link) : null,
      locked: U.locked ?? !1,
    };
    ("customData" in U || "customData" in e) &&
      (r.customData = "customData" in e ? e.customData : U.customData);
    let a = { ...U, ...r, ...Yo(r), ...e };
    return delete a.strokeSharpness, delete a.boundElementIds, a;
  },
  wo = (U) => {
    switch (((U = { ...U }), U.type)) {
      case "text":
        ...
        let e = U.fontSize,
          r = U.fontFamily;
        if ("font" in U) {
          let [b, R] = U.font.split(" ");
          (e = parseFloat(b)), (r = Vo(R));
        }
        let a = (typeof U.text == "string" && U.text) || "",
          c = U.lineHeight || (U.height ? mo(U) : yo(U.fontFamily));
        return (
          (U = k(U, {
            fontSize: e,
            fontFamily: r,
            text: a,
            textAlign: U.textAlign || oo,
            verticalAlign: U.verticalAlign || io,
            containerId: U.containerId ?? null,
            originalText: U.originalText || a,
            autoResize: U.autoResize ?? !0,
            lineHeight: c,
          })),
          !a && !U.isDeleted && ((U = { ...U, originalText: a, isDeleted: !0 }), (U = Q6(U))),
          U
        );
      case "freedraw":
        return k(U, {
      ...
      case "line":
      case "draw":
        let { startArrowhead: t = null, endArrowhead: d = null } = U,
          f = U.x,
          i = U.y,
          s =
            !Array.isArray(U.points) || U.points.length < 2
              ? [B(0, 0), B(U.width, U.height)]
              : U.points;
        return (
          (s[0][0] !== 0 || s[0][1] !== 0) &&
            ({ points: s, x: f, y: i } = Z6.getNormalizeElementPointsAndCoords(U)),
          k(U, {
            type: U.type === "draw" ? "line" : U.type,
            startBinding: tU(U, U.startBinding),
            endBinding: tU(U, U.endBinding),
            lastCommittedPoint: null,
            startArrowhead: t,
            endArrowhead: d,
            points: s,
            x: f,
            y: i,
            ...(No(U) ? { polygon: Ro(U.points) ? U.polygon ?? !1 : !1 } : {}),
            ...v6(s),
          })
        );
      case "arrow": {
        let { startArrowhead: b = null, endArrowhead: R = "arrow" } = U,
          n = U.x,
          $ = U.y,
          E =
            !Array.isArray(U.points) || U.points.length < 2
              ...
              : U.points;
        (E[0][0] !== 0 || E[0][1] !== 0) &&
          ({ points: E, x: n, y: $ } = Z6.getNormalizeElementPointsAndCoords(U));
        let o = {
          type: U.type,
          startBinding: tU(U, U.startBinding),
          endBinding: tU(U, U.endBinding),
        ...
        return OU(U)
          ? k(U, {
              ...
              startBinding: tU(U, U.startBinding),
              endBinding: tU(U, U.endBinding),
              fixedSegments: U.fixedSegments,
              startIsSpecial: U.startIsSpecial,
              endIsSpecial: U.endIsSpecial,
            })
          : k(U, o);
      }
      case "ellipse":
      ...
      case "magicframe":
      case "frame":
        return k(U, { name: U.name ?? null });
    }
    return null;
  },
  Oo = (U, e) => {
    if (U.boundElements) {
      let r = U.boundElements.slice(),
        a = new Set();
      U.boundElements = r.reduce((c, t) => {
        let d = e.get(t.id);
        if (d && !a.has(t.id)) {
          if ((a.add(t.id), d.isDeleted)) return c;
          c.push(t), se(d) && !d.containerId && (d.containerId = U.id);
        }
        return c;
      }, []);
    }
  ...
  Xo = (U, e) => {
    let r = U.containerId ? e.get(U.containerId) : null;
    if (((U.angle = ie(r) ? 0 : r?.angle ?? 0), !r)) {
      U.containerId = null;
      return;
    }
    if (!U.isDeleted && r.boundElements && !r.boundElements.find((a) => a.id === U.id)) {
      let a = (r.boundElements || (r.boundElements = [])).slice();
      a.push({ type: "text", id: U.id }), (r.boundElements = a);
    }
  },
  ho = (U, e) => {
    U.frameId && (e.get(U.frameId) || (U.frameId = null));
  },
  A6 = (U, e, r) => {
    let a = new Set(),
      c = e ? B6(e) : null,
      t = qo(
        (U || []).reduce((f, i) => {
          if (i.type !== "selection" && !_o(i)) {
            let s = wo(i);
            if (s) {
              let b = c?.get(i.id);
              b && b.version > s.version && (s = Q6(s, b.version)),
                a.has(s.id) && (s = { ...s, id: wU() }),
                a.add(s.id),
                f.push(s);
            }
          }
    ...
    if (!r?.repairBindings) return t;
    let d = B6(t);
    for (let f of t)
      f.frameId && ho(f, d),
        se(f) && f.containerId ? Xo(f, d) : f.boundElements && Oo(f, d),
        r.refreshDimensions && se(f) && Object.assign(f, Fo(f, Io(f, d), d)),
        To(f) &&
          (f.startBinding &&
            (!d.has(f.startBinding.elementId) || !ie(f)) &&
            (f.startBinding = null),
          f.endBinding && (!d.has(f.endBinding.elementId) || !ie(f)) && (f.endBinding = null));
    return t.map((f) => {
      if (OU(f) && !Jo(f) && !xo(f.points))
        return {
          ...f,
          ...uo(f, d, { points: [B(0, 0), f.points[f.points.length - 1]] }),
          index: f.index,
        };
      if (
        OU(f) &&
        f.startBinding &&
        f.endBinding &&
        f.startBinding.elementId === f.endBinding.elementId &&
        f.points.length > 1 &&
        f.points.some(([i, s]) => Math.abs(i) > 1e6 || Math.abs(s) > 1e6)
      ) {
        console.error("Fixing self-bound elbow arrow", f.id);
        let i = d.get(f.startBinding.elementId);
        return i
          ? {
              ...f,
              x: i.x + i.width / 2,
              ...
              points: [
                ...
                B(0, -10),
                B(i.width / 2 + 5, -10),
                B(i.width / 2 + 5, i.height / 2 + 5),
              ],
            }
          : (console.error("Bound element not found", f.startBinding.elementId), f);
      }
      return f;
    });
  },
  Ho = (U, e, r) => {
    let a = e[U];
    return a !== void 0 ? a : r[U];
  },
  S6 = {
    isSidebarDocked: (U, e) => [
      "defaultSidebarDockedPreference",
      U.isSidebarDocked ?? Ho("defaultSidebarDockedPreference", U, e),
    ],
  },
  Mo = (U, e) => {
    U = U || {};
    ...
    for (let [c, t] of Object.entries(r)) {
      let d = U[c],
        f = e ? e[c] : void 0;
      a[c] = d !== void 0 ? d : f !== void 0 ? f : t;
    }
    return {
      ...a,
      cursorButton: e?.cursorButton || "up",
      penDetected: e?.penDetected ?? (U.penMode ? U.penDetected ?? !1 : !1),
      activeTool: {
        ...Eo(r, a.activeTool.type && Lo[a.activeTool.type] ? a.activeTool : { type: "selection" }),
        lastActiveTool: null,
        locked: a.activeTool.locked ?? !1,
      },
      zoom: { value: vU(oe(U.zoom) ? U.zoom : U.zoom?.value ?? r.zoom.value) },
      openSidebar: typeof U.openSidebar == "string" ? { name: so.name } : a.openSidebar,
      gridSize: ZU(oe(U.gridSize) ? U.gridSize : bo),
      gridStep: SU(oe(U.gridStep) ? U.gridStep : no),
      editingFrame: null,
    };
  },
  G6 = (U, e, r, a) => ({
    elements: A6(U?.elements, r, a),
    appState: Mo(U?.appState, e || null),
    files: U?.files || {},
  }),
  W6 = (U) => {
    let e = A6(go(U.elements), null);
    return e.length ? { ...U, elements: e } : null;
  },
  C6 = (U = [], e) => {
    let r = [];
    for (let a of U)
      if (Array.isArray(a)) {
        let c = W6({ status: e, elements: a, id: wU(), created: Date.now() });
        c && r.push(c);
      } else {
        let c = a,
          t = W6({
...
var k6 = async (U) => {
  ...
  if (U.type === O.png)
    try {
      return await (await import("./data/image-HXEZYJPQ.js")).decodePngMetadata(U);
    } catch (r) {
      throw r.message === "INVALID"
        ? new H("Image doesn't contain scene", "IMAGE_NOT_CONTAINS_SCENE_DATA")
        : new H("Error: cannot restore image");
    }
  else if (
    ("text" in Blob
      ? (e = await U.text())
      : (e = await new Promise((r) => {
          ...
          a.readAsText(U, "utf8"),
            (a.onloadend = () => {
              a.readyState === FileReader.DONE && r(a.result);
            });
        })),
    U.type === O.svg)
  )
...
var Qo = (U) => (U && U.name.match(/\.(json|excalidraw|png|svg)$/)?.[1]) || null,
  Yy = (U) => U === "png" || U === "svg",
  Te = (U) => {
    let e = Qo(U);
    return e === "png" || e === "svg";
  },
  Ao = (U) => !!U && Object.values(vo).includes(U),
  hU = (U) => {
    let { type: e } = U || {};
    return Ao(e);
  },
  Go = async (U, e, r, a) => {
    ...
    try {
      try {
        ...
        throw hU(U) ? new H("Image doesn't contain scene", "IMAGE_NOT_CONTAINS_SCENE_DATA") : d;
      }
      if (Ke(t))
        return {
          type: O.excalidraw,
          data: G6(
            {
              elements: Wo(t.elements || []),
              appState: {
                theme: e?.theme,
                fileHandle: a || U.handle || null,
                ...gU(t.appState || {}),
                ...(e ? MU(t.elements || [], e) : {}),
              },
              files: t.files,
      ...
      if (AU(t)) return { type: O.excalidrawlib, data: t };
      throw new Error("Error: invalid file");
    } catch (d) {
      throw d instanceof H ? d : new Error("Error: invalid file");
    }
  },
  Ne = async (U, e, r, a) => {
    let c = await Go(U, e, r, a);
    if (c.type !== O.excalidraw) throw new Error("Error: invalid file");
    return c.data;
  },
  Co = (U, e = "unpublished") => {
    let r = JSON.parse(U);
    if (!AU(r)) throw new Error("Invalid library");
    let a = r.libraryItems || r.library;
    return C6(a, e);
  },
  _y = async (U, e = "unpublished") => Co(await k6(U), e),
  Ly = async (U) =>
    new Promise(async (e, r) => {
      try {
        So(U) && (U = await U),
          U.toBlob((a) => {
            if (!a) return r(new RU("Error: Canvas too big", "CANVAS_POSSIBLY_TOO_BIG"));
            e(a);
          });
      } catch (a) {
        r(a);
  ...
  Vy = async (U) => {
    try {
      let e = await window.crypto.subtle.digest("SHA-1", await M(U));
      return Zo(new Uint8Array(e));
    } catch (e) {
      return console.error(e), Bo(40);
    }
  },
  wy = async (U) =>
    new Promise((e, r) => {
  ...
  Oy = (U, e) => `data:${e};base64,${uU(iU(U), !0)}`;
var Xy = (U) => xU(U.slice(U.indexOf(",") + 1)),
  hy = async (U, e) => {
    if (U.type === O.svg) return U;
    let [r, a] = await Promise.all([
        import("pica").then((t) => t.default),
        import("image-blob-reduce").then((t) => t.default),
      ]),
      c = a({ pica: r({ features: ["js", "wasm"] }) });
    if (e.outputType) {
      let { outputType: t } = e;
      c._create_blob = function (d) {
        return this.pica.toBlob(d.out_canvas, t, 0.8).then((f) => ((d.out_blob = f), d));
      };
    }
    if (!hU(U)) throw new Error("Error: unsupported file type", { cause: "UNSUPPORTED" });
    return new File([await c.toBlob(U, { max: e.maxWidthOrHeight, alpha: !0 })], U.name, {
      type: e.outputType || U.type,
    });
  },
  Hy = (U, e = "") => new File([new TextEncoder().encode(U)], e, { type: O.svg }),
  My = async (U, e = "") => {
    let r;
    try {
      r = await fetch(U);
    ...
    if (!r.ok) throw new Error("Error: failed to fetch image", { cause: "FETCH_ERROR" });
    let a = await r.blob();
    if (a.type && hU(a)) {
      let c = e || a.name || "";
      return new File([a], c, { type: a.type });
    }
    throw new Error("Error: unsupported file type", { cause: "UNSUPPORTED" });
  },
  By = async (U) => {
    let e = U.dataTransfer.files.item(0),
      r = await Po(U);
    return { file: e ? await GU(e) : null, fileHandle: r };
  },
  Po = async (U) => {
    if (ue)
      try {
        return (await U.dataTransfer.items[0].getAsFileSystemHandle()) || null;
      } catch (e) {
  ...
  P6 = (U) => {
    let e = null,
      r = `${[...new Uint8Array(U).slice(0, 8)].join(" ")} `,
      a = { png: "137 80 78 71 13 10 26 10 ", jpg: "255 216 255 ", gif: "71 73 70 56 57 97 " };
    return (
      r === a.png
        ? (e = O.png)
        : r.startsWith(a.jpg)
        ? (e = O.jpg)
        : r.startsWith(a.gif) && (e = O.gif),
      e
    );
  },
  XU = (U, e, r) => new File([U], r || "", { type: e }),
  GU = async (U) => {
    if (U.type) {
      if (hU(U)) {
        let e = await M(U),
          r = P6(e);
        r && r !== U.type && (U = XU(e, r, U.name));
      }
    } else if (U?.name?.endsWith(".excalidrawlib")) U = XU(await M(U), O.excalidrawlib, U.name);
    else if (U?.name?.endsWith(".excalidraw")) U = XU(await M(U), O.excalidraw, U.name);
    else {
      let e = await M(U),
        r = P6(e);
      r && (U = XU(e, r, U.name));
  ...
  M = (U) =>
    "arrayBuffer" in U
      ...
      : new Promise((e, r) => {
          let a = new FileReader();
          (a.onload = (c) => {
            if (!c.target?.result) return r(new Error("Couldn't convert blob to ArrayBuffer"));
            e(c.target.result);
          }),
            a.readAsArrayBuffer(U);
        });
var zo = async (U) => {
    let r = z6(new Uint8Array(await M(U))).find((a) => a.name === "tEXt");
    return r ? j6.decode(r.data) : null;
  },
  Py = async ({ blob: U, metadata: e }) => {
    let r = z6(new Uint8Array(await M(U))),
      a = j6.encode(be.excalidraw, JSON.stringify(IU({ text: e, compress: !0 })));
    return r.splice(-1, 0, a), new Blob([ko(r)], { type: be.png });
  },
  ky = async (U) => {
    let e = await zo(U);
    if (e?.keyword === be.excalidraw)
      try {
        let r = JSON.parse(e.text);
        if (!("encoded" in r)) {
          if ("type" in r && r.type === jo.excalidraw) return e.text;
          throw new Error("FAILED");
        }
        return mU(r);
      } catch (r) {
        throw (console.error(r), new Error("FAILED"));
      }
    throw new Error("INVALID");
  };
export {
