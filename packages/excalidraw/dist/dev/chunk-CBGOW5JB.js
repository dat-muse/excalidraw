...
import { promiseTry as promiseTry2, LOCAL_FONT_PROTOCOL as LOCAL_FONT_PROTOCOL2 } from "@excalidraw/common";
...
var _renderStaticScene = ({
  ...
  visibleElements.filter((el) => !isIframeLikeElement(el)).forEach((element) => {
    try {
      ...
      if (isTextElement2(element) && element.containerId && elementsMap.has(element.containerId)) {
        return;
      }
      context.save();
...
var renderSceneToSvg = (elements, elementsMap, rsvg, svgRoot, files, renderConfig) => {
  ...
  elements.filter((el) => !isIframeLikeElement2(el)).forEach((element) => {
    if (!element.isDeleted) {
      if (isTextElement3(element) && element.containerId && elementsMap.has(element.containerId)) {
        return;
      }
      try {
        renderElementToSvg(
  ...
  for (const element of elements) {
    if (isFrameLikeElement(element)) {
      let textElement = newTextElement({
        ...
        strokeColor: opts.exportWithDarkMode ? FRAME_STYLE3.nameColorDarkTheme : FRAME_STYLE3.nameColorLightTheme,
    ...
    container.boundElements = boundElements.reduce(
      (acc, binding) => {
        ...
        if (boundElement && !boundIds.has(binding.id)) {
          ...
          acc.push(binding);
          if (isTextElement4(boundElement) && // being slightly conservative here, preserving existing containerId
...
var restoreElements = (elements, localElements, opts) => {
  ...
  for (const element of restoredElements) {
    ...
    if (isLinearElement(element)) {
      if (element.startBinding && (!restoredElementsMap.has(element.startBinding.elementId) || !isArrowElement2(element))) {
        element.startBinding = null;
      }
      if (element.endBinding && (!restoredElementsMap.has(element.endBinding.elementId) || !isArrowElement2(element))) {
